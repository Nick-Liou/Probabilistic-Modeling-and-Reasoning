...
def main() -> None:   
    ...
    (_ , CPT_tables) = parse_file(selected_file)

    print("\n\nQuestion 2: \n")
    prob = calculate_cond_prob(A,a,B,b , CPT_tables)

    t1 = ', '.join([f"X_{A_i} = {a_i}" for A_i, a_i in zip(A, a)])
    t2 = ', '.join([f"X_{B_i} = {b_i}" for B_i, b_i in zip(B, b)])
    
    print(f"P({t1}{' | '*bool(B)}{t2}) = {prob}")
    

    
    # Load new CPT for the next questions
    selected_file = "printer_beta_prior.txt"    
    ...
    # Loop through all possible combinations of 5 elements (0 or 1)
    ...
    print("\n\nMost likely diagonsis:")

    t1 = ', '.join([f"X_{A_i} = {a_i}" for A_i, a_i in zip(A, diagnostics)])
    t2 = ', '.join([f"X_{B_i} = {b_i}" for B_i, b_i in zip(B, b)])
    
    print(f"P({t1}{' | '*bool(B)}{t2}) = {max_prop}")
